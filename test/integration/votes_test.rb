...
class VotesTest < ActionDispatch::IntegrationTest
  test 'in_favor' do
    refute proposals(:proposal1).voted?(users(:one), :in_favor)

    sign_in users(:one)
    post in_favor_proposal_path(id: proposals(:proposal1).id)

    assert proposals(:proposal1).voted?(users(:one), :in_favor)
  end

  test 'opposed' do
    refute proposals(:proposal1).voted?(users(:one), :opposed)

    sign_in users(:one)
    post opposed_proposal_path(id: proposals(:proposal1).id)

    assert proposals(:proposal1).voted?(users(:one), :opposed)
  end
  ...
  test 'unvote' do
    ...
    proposals(:proposal1).reload
    refute proposals(:proposal1).voted?(users(:one))
  end
end
